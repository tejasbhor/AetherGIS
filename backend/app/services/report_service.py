...
def generate_html_report(
    ...
    for a in (alerts or [])[:25]:
        alert_rows += f"""
        <tr>
          <td class="font-mono">{_escape(a.get('frame_index', '—'))}</td>
          <td>{_escape(str(a.get('type', '—')).replace('_', ' ').capitalize())}</td>
          <td>{_sev_badge(a.get('severity', 'low'))}</td>
          <td style="color: #444444;">{_escape(str(a.get('description', '—'))[:140])}</td>
        </tr>"""
    ...
    for t in (trajectories or [])[:15]:
        traj_rows += f"""
        <tr>
          <td class="font-mono">{_escape(t.get('id', '—'))}</td>
    ...
    for iss in (consistency_issues or [])[:20]:
        issue_rows += f"""
        <tr>
          <td class="font-mono">{_escape(iss.get('frame', '—'))}</td>
          <td>{_escape(iss.get('issue', '—'))}</td>
          <td>{_sev_badge(iss.get('severity', 'low'))}</td>
          <td class="font-mono">{_escape(iss.get('mad_score', '—'))}</td>
        </tr>"""
    ...
    for model, count in sorted(frame_stats["by_model"].items(), key=lambda x: x[1], reverse=True):
        ...
        model_rows += f"<tr><td class='font-mono'>{_escape(model)}</td><td class='font-mono'>{count}</td><td class='font-mono'>{pct:.1f}%</td></tr>"
    
    # Confidence distribution
    frame_stats["by_confidence"]
    
    safe_job_id = _escape(job_id)
    safe_job_id_short = _escape(job_id[:12])
    safe_job_id_mid = _escape(job_id[:16])
...
<head>
  ...
  <title>AetherGIS Technical Report — {safe_job_id_short}</title>
    ...
    <div class="report-header">
      ...
      <div class="report-meta">
        ...
        <span><span class="meta-label">Status:</span> 
          <span class="status-badge {'status-success' if status.lower() == 'completed' else 'status-error' if error_msg else 'status-warning'}">
    ...
    <div class="report-footer">
      <div class="footer-section">
        ...
        <p class="footer-text">
          ...
          <strong>Report ID:</strong> RPT-{safe_job_id_short}-{datetime.utcnow().strftime('%Y%m%d')}
        </p>
      </div>
      
      <div class="footer-meta">
        <span>AetherGIS Technical Report</span>
        <span>Job: {safe_job_id_mid}</span>
