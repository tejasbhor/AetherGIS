...
def generate_html_report(
    ...
    status_raw = str(pipeline_result.get("status", "Unknown"))
    status = html.escape(status_raw)
    bbox = html.escape(str(pipeline_result.get("bbox", [])))
    ...
    for model, count in sorted(frame_stats["by_model"].items(), key=lambda x: x[1], reverse=True):
        ...
        model_escaped = html.escape(str(model))
        model_rows += f"<tr><td class='font-mono'>{model_escaped}</td><td class='font-mono'>{count}</td><td class='font-mono'>{pct:.1f}%</td></tr>"
    
    # Confidence distribution
    frame_stats["by_confidence"]
    
    job_id_escaped_short = html.escape(job_id[:12])
    job_id_escaped_med = html.escape(job_id[:16])
...
<head>
  ...
  <title>AetherGIS Technical Report — {job_id_escaped_short}</title>
    ...
    <div class="report-header">
      ...
      <div class="report-meta">
        ...
        <span><span class="meta-label">Status:</span> 
          <span class="status-badge {'status-success' if status_raw == 'completed' else 'status-error' if error_msg else 'status-warning'}">
    ...
    <div class="report-footer">
      <div class="footer-section">
        ...
        <p class="footer-text">
          ...
          <strong>Report ID:</strong> RPT-{job_id_escaped_short}-{datetime.utcnow().strftime('%Y%m%d')}
        </p>
      </div>
      
      <div class="footer-meta">
        <span>AetherGIS Technical Report</span>
        <span>Job: {job_id_escaped_med}</span>
