...
def generate_html_report(
    ...
    error_msg_raw = pipeline_result.get("error")
    error_msg = html.escape(str(error_msg_raw)) if error_msg_raw else None
    ...
    for a in (alerts or [])[:25]:
        alert_type = html.escape(str(a.get('type', '—')).replace('_', ' ').capitalize())
    ...
    for iss in (consistency_issues or [])[:20]:
        issue_desc = html.escape(str(iss.get('issue', '—')))
        frame_idx = html.escape(str(iss.get('frame', '—')))
        mad_score = html.escape(str(iss.get('mad_score', '—')))
        issue_rows += f"""
        <tr>
          <td class="font-mono">{frame_idx}</td>
          <td>{issue_desc}</td>
          <td>{_sev_badge(iss.get('severity', 'low'))}</td>
          <td class="font-mono">{mad_score}</td>
        </tr>"""
    ...
    for model, count in sorted(frame_stats["by_model"].items(), key=lambda x: x[1], reverse=True):
        ...
        model_rows += f"<tr><td class='font-mono'>{escaped_model}</td><td class='font-mono'>{count}</td><td class='font-mono'>{pct:.1f}%</td></tr>"
