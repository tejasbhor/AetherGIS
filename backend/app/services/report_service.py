...
def generate_html_report(
    ...
    job_id = escape(str(job_id))
    metrics = pipeline_result.get("metrics") or {}
    frames = pipeline_result.get("frames") or []
    
    # Job metadata
    layer_id = escape(str(pipeline_result.get("layer_id", "Unknown")))
    data_source = escape(str(pipeline_result.get("data_source", "Unknown")))
    status = escape(str(pipeline_result.get("status", "Unknown")))
    ...
    error_msg = escape(str(error_val)) if error_val else None
    ...
    for a in (alerts or [])[:25]:
        alert_rows += f"""
        <tr>
          <td class="font-mono">{escape(str(a.get('frame_index', '—')))}</td>
          <td>{escape(str(a.get('type', '—')).replace('_', ' ').capitalize())}</td>
          <td>{_sev_badge(a.get('severity', 'low'))}</td>
          <td style="color: #444444;">{escape(str(a.get('description', '—'))[:140])}</td>
        </tr>"""
    ...
    for t in (trajectories or [])[:15]:
        traj_rows += f"""
        <tr>
          <td class="font-mono">{escape(str(t.get('id', '—')))}</td>
    ...
    for iss in (consistency_issues or [])[:20]:
        issue_rows += f"""
        <tr>
          <td class="font-mono">{escape(str(iss.get('frame', '—')))}</td>
          <td>{escape(str(iss.get('issue', '—')))}</td>
          <td>{_sev_badge(iss.get('severity', 'low'))}</td>
          <td class="font-mono">{escape(str(iss.get('mad_score', '—')))}</td>
        </tr>"""
    ...
    for model, count in sorted(frame_stats["by_model"].items(), key=lambda x: x[1], reverse=True):
        ...
        model_rows += f"<tr><td class='font-mono'>{escape(str(model))}</td><td class='font-mono'>{count}</td><td class='font-mono'>{pct:.1f}%</td></tr>"
      ...
      <div class="section">
        ...
        <div class="info-grid">
          ...
          <div class="info-row">
            ...
            <div class="info-value">{escape(_format_datetime(created_at))}</div>
          </div>
          <div class="info-row">
            <div class="info-label">Completed At</div>
            <div class="info-value">{escape(_format_datetime(completed_at))}</div>
          </div>
          <div class="info-row">
      ...
      <div class="section">
        ...
        <div class="info-grid">
          <div class="info-row">
            ...
            <div class="info-value">{escape(_format_datetime(time_start))}</div>
          </div>
          <div class="info-row">
            <div class="info-label">End Time</div>
            <div class="info-value">{escape(_format_datetime(time_end))}</div>
          </div>
        </div>
