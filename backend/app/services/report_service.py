...
def generate_html_report(
    ...
    for a in (alerts or [])[:25]:
        ...
        a_type = html_lib.escape(str(a.get('type', '—')).replace('_', ' ').capitalize())
        frame_idx = html_lib.escape(str(a.get('frame_index', '—')))
        alert_rows += f"""
        <tr>
    ...
    for t in (trajectories or [])[:15]:
        t_id = html_lib.escape(str(t.get('id', '—')))
        traj_rows += f"""
        <tr>
    ...
    for iss in (consistency_issues or [])[:20]:
        iss_frame = html_lib.escape(str(iss.get('frame', '—')))
        iss_type = html_lib.escape(str(iss.get('issue', '—')))
        iss_score = html_lib.escape(str(iss.get('mad_score', '—')))
        issue_rows += f"""
        <tr>
          <td class="font-mono">{iss_frame}</td>
          <td>{iss_type}</td>
          <td>{_sev_badge(iss.get('severity', 'low'))}</td>
          <td class="font-mono">{iss_score}</td>
        </tr>"""
