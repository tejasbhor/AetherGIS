...
def generate_html_report(
    ...
    status = html.escape(str(pipeline_result.get("status", "Unknown")).upper())
    ...
    error_val = pipeline_result.get("error")
    error_msg = html.escape(str(error_val)) if error_val else None
    ...
    for a in (alerts or [])[:25]:
        desc = html.escape(str(a.get('description', '—'))[:140])
        a_type = html.escape(str(a.get('type', '—')).replace('_', ' ').capitalize())
        a_frame = html.escape(str(a.get('frame_index', '—')))
        alert_rows += f"""
        <tr>
          <td class="font-mono">{a_frame}</td>
          <td>{a_type}</td>
    ...
    for iss in (consistency_issues or [])[:20]:
        i_frame = html.escape(str(iss.get('frame', '—')))
        i_issue = html.escape(str(iss.get('issue', '—')))
        i_mad = html.escape(str(iss.get('mad_score', '—')))
        issue_rows += f"""
        <tr>
          <td class="font-mono">{i_frame}</td>
          <td>{i_issue}</td>
          <td>{_sev_badge(iss.get('severity', 'low'))}</td>
          <td class="font-mono">{i_mad}</td>
        </tr>"""
    ...
    for model, count in sorted(frame_stats["by_model"].items(), key=lambda x: x[1], reverse=True):
        ...
        m_name = html.escape(str(model))
        model_rows += f"<tr><td class='font-mono'>{m_name}</td><td class='font-mono'>{count}</td><td class='font-mono'>{pct:.1f}%</td></tr>"
...
<head>
  ...
  <title>AetherGIS Technical Report — {html.escape(job_id[:12])}</title>
  ...
  <div class="container">
    ...
    <div class="report-header">
      ...
      <div class="report-meta">
        <span><span class="meta-label">Job ID:</span> {html.escape(job_id)}</span>
        <span><span class="meta-label">Layer:</span> {layer_id}</span>
        <span><span class="meta-label">Status:</span> 
          <span class="status-badge {'status-success' if pipeline_result.get('status') == 'completed' else 'status-error' if pipeline_result.get('error') else 'status-warning'}">
    ...
    <div class="content">
      ...
      <div class="section">
        ...
        <div class="info-grid">
          <div class="info-row">
            ...
            <div class="info-value">{html.escape(job_id)}</div>
          </div>
          <div class="info-row">
      ...
      <div class="section">
        ...
        <ul class="artifact-list">
          <li>
            <span class="artifact-path">/exports/{html.escape(job_id)}/original.mp4</span>
            <span class="artifact-desc">Original observed frame sequence (no interpolation)</span>
          </li>
          <li>
            <span class="artifact-path">/exports/{html.escape(job_id)}/interpolated.mp4</span>
        ...
        <ul class="artifact-list">
          <li>
            <span class="artifact-path">/exports/{html.escape(job_id)}/frames/frame_*.png</span>
        ...
        <ul class="artifact-list">
          <li>
            <span class="artifact-path">/exports/{html.escape(job_id)}/metadata.json</span>
            <span class="artifact-desc">Complete frame metadata with per-frame metrics</span>
          </li>
          <li>
            <span class="artifact-path">/exports/{html.escape(job_id)}/report.html</span>
      ...
      <div class="section">
        ...
        <p style="font-size: 11px; color: var(--t3); line-height: 1.6;">
          ...
          are computed from the actual execution artifacts stored at <code>/exports/{html.escape(job_id)}/</code>.
    ...
    <div class="report-footer">
      <div class="footer-section">
        ...
        <p class="footer-text">
          ...
          <strong>Report ID:</strong> RPT-{html.escape(job_id[:12])}-{datetime.utcnow().strftime('%Y%m%d')}
        </p>
      </div>
      
      <div class="footer-meta">
        <span>AetherGIS Technical Report</span>
        <span>Job: {html.escape(job_id[:16])}</span>
