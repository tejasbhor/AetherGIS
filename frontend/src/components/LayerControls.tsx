...
export default function LayerControls() {
    ...
    if (layersLoading) {
      return (
        <div style={{ padding: '8px' }}>
          ...
          <div style={{ fontFamily: 'var(--mono)', fontSize: 9, color: 'var(--t4)', marginTop: 4, fontStyle: 'italic' }}>Fetching layers from NASA GIBS...</div>
        </div>
      );
    }
  ...
  return (
    <>
      ...
      <div style={{ maxHeight: 220, overflowY: 'auto', borderBottom: '1px solid var(--b3)' }}>{renderLayerList()}</div>
      ...
      <div className="section-hdr">AI / Interpolation<span style={{ fontSize: 8 }}>v</span></div>
      <div className="section-body">
        <div className="form-row"><span className="form-label">Model</span><select className="inp" value={interpolationModel} onChange={(e) => setInterpolationModel(e.target.value as 'film' | 'rife' | 'dain' | 'lk_fallback')}><option value="film">FILM (Primary)</option><option value="rife">RIFE 4.x</option><option value="dain">DAIN</option><option value="lk_fallback">Optical Flow Baseline</option></select></div>
      ...
      {/* ── Advanced Overlays (Modules 1–10) ──────────────────────────────── */}
