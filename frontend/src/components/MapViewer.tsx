...
function ConfirmDialog({
  ...
  return (
    <div className="cdlg-backdrop" style={{ zIndex: 2000 }}>
      <div className="cdlg-box" style={{ maxWidth: 320 }}>
        <div className="cdlg-header" style={{ borderLeft: '3px solid var(--red)', background: 'var(--red-bg)' }}>
          <span className="cdlg-icon" style={{ color: 'var(--red)' }}>⚠</span>
          <span className="cdlg-title">{title}</span>
        </div>
        <div className="cdlg-body" style={{ color: 'var(--t2)', fontSize: 11, lineHeight: 1.4 }}>{message}</div>
        <div className="cdlg-footer">
          <button className="cdlg-btn cancel" onClick={onCancel}>Cancel</button>
          <button className="cdlg-btn confirm" style={{ background: 'var(--red)', borderColor: 'var(--red)' }} onClick={onConfirm}>Discard</button>
        </div>
      </div>
    </div>
  );
...
export default function MapViewer() {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden' }}>
      ...
      {showHUD && (
        <div className="map-hud">
          ...
          {bbox && layerLegendText && (
            <div className="hud-panel" style={{ maxWidth: 280 }}>
              {layerLegendText}
            </div>
          )}
      ...
      {!bbox && (
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', background: 'rgba(10,16,28,0.95)', border: '1px solid rgba(64,180,255,0.25)', padding: '16px 32px', textAlign: 'center', pointerEvents: 'none', backdropFilter: 'blur(16px)', boxShadow: '0 20px 60px rgba(0,0,0,0.8)' }}>
          <div style={{ fontSize: 13, color: '#fff', fontFamily: 'var(--cond)', fontWeight: 700, letterSpacing: '0.15em' }}>AETHER GIS CORE READY</div>
          <div style={{ fontSize: 9, color: '#707788', marginTop: 8, fontFamily: 'var(--mono)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Awaiting AOI selection to initialize telemetry stream</div>
        </div>
      )}

      {/* BOTTOM: Status Bar */}
      {bbox && (
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'rgba(10,16,28,0.96)', borderTop: '1px solid rgba(255,255,255,0.08)', height: 26, display: 'flex', alignItems: 'center', padding: '0 12px', fontFamily: 'var(--mono)', fontSize: 9, letterSpacing: '0.02em', zIndex: 1001 }}>
          <span style={{ color: '#444', fontWeight: 700, marginRight: 16 }}>SENSOR VIEWPORT</span>
